...
public class SwaggerConfig {
	...
	public Docket postsApi() {
		return new Docket(DocumentationType.SWAGGER_2).groupName("ssafyVueMovieWS").apiInfo(apiInfo()).select()
				.apis(RequestHandlerSelectors.basePackage("com.ssafy.ws.controller"))
				.paths(PathSelectors.ant("/movie/**")).build();
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("SSAFY API")
				.description("<h2>SSAFY VUE Movie API Reference for Developers<h2>")
